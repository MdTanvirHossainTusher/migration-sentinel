...
| `./gradlew sandboxTest` → `Could not find a valid Docker environment` on **Windows + Docker Desktop 29** | Known testcontainers/docker-java issue with Docker 29's named pipe. Run the sandbox tests on Linux (or WSL2), or use `docker compose up` which is unaffected. CI runs them on `ubuntu-latest`. |
